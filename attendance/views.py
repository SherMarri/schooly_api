...
class DailyStaffAttendanceViewSet(
    CreateModelMixin, ListModelMixin,
    RetrieveModelMixin, GenericViewSet,
    UpdateModelMixin):
    serializer_class = serializers.DailyStaffAttendanceSerializer
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        # if 'download' in params and params['download'] == 'true':
        #     return self.get_downloadable_link(instance)
        serializer = serializers.DailyStaffAttendanceDetailsSerializer(
            instance=instance
        )
        return Response(status=status.HTTP_200_OK, data=serializer.data)
    ...
    def update(self, request, *args, **kwargs):
        """
        For updating items of a daily staff attendance entry
        """
        data = request.data
        instance = self.get_object()
        if 'items' in data:
            ...
            items = items.values()
            if len(items) > 0:
                models.StaffAttendanceItem.objects.bulk_update(
                    items, ['status', 'comments']
                )
                instance.average_attendance = self.get_average_attendance(items)
                instance.save()
    ...
    def get_average_attendance(items):
        present = reduce(
            lambda x, y: x + 1 if y.status == models.StaffAttendanceItem.PRESENT else x,
            items, 0.0
        )
    ...
    # @staticmethod
    # def get_downloadable_link(instance):
    #     instance = models.DailyStaffAttendance.objects.filter(id=instance.id).prefetch_related(
    #         'items__employee__profile__staff_info').first()
    #     timestamp = datetime.datetime.now().strftime("%f")
    #     date = instance.date.strftime('%Y_%m_%d')
    #     file_name = f'staff_attendance_{date}_{timestamp}.csv'
    #     with open(os.path.join(settings.BASE_DIR, f'downloadables/{file_name}'), mode='w') as file:
    #         writer = csv.writer(file, delimiter=',')
    #         writer.writerow([
    #             'Full Name', 'Status', 'Comments'
    #         ])
    #         for attendance_item in instance.items.all():
    #             writer.writerow(DailyStaffAttendanceViewSet.get_csv_row(attendance_item))
    #     return Response(file_name, status=status.HTTP_200_OK)

    # @staticmethod
    # def get_csv_row(item):
    #     status = None
    #     if item.status == models.StaffAttendanceItem.PRESENT:
    #         status = 'P'
    #     elif item.status == models.StaffAttendanceItem.ABSENT:
    #         status = 'A'
    #     elif item.status == models.StaffAttendanceItem.LEAVE:
    #         status = 'L'
    #     return [
    #         item.employee.profile.fullname,
    #         status,
    #         item.comments
    #     ]
